...
namespace NxRebuild.Client.Pages.NxPrograms.MDI.Tree_List_View {
    ...
    public abstract class AknTreeViewBase<TKey> : ComponentBase {
        ...
        [Parameter] public EventCallback<MyTreeData<TKey>> OnNodeSelected { get; set; }
        [Parameter] public EventCallback<(MyTreeData<TKey> Target, MyDataObj<TKey>? DroppedItem, MyTreeData<TKey>? DroppedNode)> OnNodeDropped { get; set; }

        private MyTreeData<TKey>? _editingNode;
        public MyTreeData<TKey>? editingNode => _editingNode; 
        ...
        protected async Task SelectTreeNode(MyTreeData<TKey> node) {
            if (node == null) return;
        ...
        private void ClearTreeSelection(List<MyTreeData<TKey>> nodes) {
        ...
        public async Task StartEditingTreeNode(MyTreeData<TKey> node) {
        ...
        public void StopEditingTreeNode(MyTreeData<TKey> node, bool isCommit) {
            if (isCommit && !string.IsNullOrWhiteSpace(node.EditingText)) node.Text = node.EditingText;
            node.IsEditing = false;
            base.StateHasChanged();
        }

        public void HandleTreeKeyUp(KeyboardEventArgs e, MyTreeData<TKey> node) {
            if (e.Key == "Enter") StopEditingTreeNode(node, true);
            if (e.Key == "Escape") StopEditingTreeNode(node, false);
        }


        protected async Task DropOnNode(MyTreeData<TKey> targetNode) {
            ...
            if (OnNodeDropped.HasDelegate) {
                await OnNodeDropped.InvokeAsync((targetNode, DraggingState<TKey>.DraggingGridItem, DraggingState<TKey>.DraggingTreeNode));
            }
